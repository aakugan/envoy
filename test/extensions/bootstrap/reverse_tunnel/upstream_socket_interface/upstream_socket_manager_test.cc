...
TEST_F(TestUpstreamSocketManager, MarkSocketDeadCallsReportDisconnection) {
  ...
  EXPECT_CALL(*reporter_ptr,
              reportDisconnectionEvent(testing::Eq(node_id), testing::Eq(cluster_id), testing::Eq(fd)));
  socket_manager_->markSocketDead(fd);
}

TEST_F(TestUpstreamSocketManager, ReportGoAwayCallsReporterGoAwayEvent) {
  socket_manager_.reset();
  ...
  socket_manager_->reportGoAway(fd);
}

TEST_F(TestUpstreamSocketManager, ReportGoAwayUnknownFdLogsWarning) {
  socket_manager_->reportGoAway(999);
}
